...
class AlertService {
  ...
  */
  ...
  /*
    This will prevent any slow connection problem
    This will look up if this connection is retrying
    It will prevent to stack up the memory
  */

  private timeoutPolicy = timeout(50, TimeoutStrategy.Cooperative);
  private circuitBreakerPolicy = circuitBreaker(handleAll, {
    halfOpenAfter: 30 * 1000,
    breaker: new ConsecutiveBreaker(3),
  });
  private circuitBreaker = wrap(this.timeoutPolicy, this.circuitBreakerPolicy);

  constructor() {
    this.subscriber = redisClient.duplicate();

    this.circuitBreakerPolicy.onStateChange((state) => {
      logger.warn('Circuit breaker state', { state });
    });
  ...
  */
  private async handleNewPrice(message: string) {
    ...
    try {
      ...
      const triggeredAlertIds = await this.circuitBreaker.execute(() =>
        AlertModel.triggeredAlert(symbol, price)
      );
      ...
      const BATCH_SIZE = 15;
      for (let i=0; i<newAlertIds.length; i+=BATCH_SIZE) {
        const chunk = newAlertIds.slice(i, i+BATCH_SIZE);
        await Promise.all(chunk.map (alertId => {
          this.processTriggeredAlert(alertId, symbol, price)
          .catch(err=> logger.error('Error', { alertId, error: err.message }))
        }));
      }
      ...
      //   .catch(err=> {
      //     try {
      //       logger.error('Unhandled error in alert processing', {
      //         alertId,
      //         symbol,
      //         error: err.message
      //       });
      //     } catch (error: any) {
      //       logger.error('Logger failed', { error: error.message })
      //     }
      //   });
      // });
    } catch (error) {
  ...
  */
  private sendTelegramNotification = async(
    ...
    if (this.telegramQueue.length >= 5000) {
      logger.warn('Warning: Massive alert hit the system!');
      return;
    }
    const message = 
        `🎯 <b>TARGET REACHED</b>\n` +
  ...
  public async createAlert(alertData: Prisma.AlertUncheckedCreateInput) {
    try {
      const newAlert = await prisma.$transaction(async (tx) => {
        const existingAlertCount = await tx.alert.count({
          where: {
        ...
        if (existingAlertCount >= MAX_ALERTS_SYMBOL) {
          throw new Error(`Maximum ${MAX_ALERTS_SYMBOL} active alerts reached`);
        }

        return await tx.alert.create({ data: alertData });
      });
